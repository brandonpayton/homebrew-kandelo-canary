require (Tap.fetch("brandonpayton", "kandelo-canary").path/"Kandelo/formula_support/kandelo_formula_support").to_s

class M4Canary < Formula
  include KandeloFormulaSupport

  desc "Independent GNU macro processor canary for Kandelo"
  ...
  # WHY: the canary must coexist with the core tap's M4. Keeping this Formula
  # keg-only lets stock Homebrew pour an independent third-party bottle
  # without competing for the global `bin/m4` link.
  keg_only "it is an independent third-party installation canary"
  ...
  test do
    ...
    dash = testpath/"dash"
    dash.binwrite(
      (formula_opt_bin("kandelo-dev/tap-core/dash")/"dash").binread,
    )
    dash.chmod 0755
